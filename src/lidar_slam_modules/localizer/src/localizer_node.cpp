...
#include <geometry_msgs/msg/pose_with_covariance_stamped.hpp> // 新增：为了接收RViz的初始位姿
...
public:
    LocalizerNode() : Node("localizer_node")
    {
        ...
        if (!m_config.map_path.empty() && std::filesystem::exists(m_config.map_path)) {
            if (m_localizer->loadMap(m_config.map_path)) {
                RCLCPP_INFO(this->get_logger(), "✅ Global Map automatically loaded from: %s", m_config.map_path.c_str());
            } else {
                RCLCPP_ERROR(this->get_logger(), "❌ Failed to load map from: %s", m_config.map_path.c_str());
            }
        } else {
            RCLCPP_WARN(this->get_logger(), "⚠️ Map path is empty or file does not exist! Path: %s", m_config.map_path.c_str());
        }
        // =============================================================
        ...
        // ================= 新增：监听 RViz 的 2D Pose Estimate 按钮 =================
        ...
        // ================= 修改：将发图的 QoS 改为 transient_local，治好 RViz 黑屏 =================
        rclcpp::QoS map_qos(1);
        map_qos.transient_local();
        m_map_cloud_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("map_cloud", map_qos);
    ...
    void initialPoseCB(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
    {
        ...
        m_state.initial_guess.block<3, 1>(0, 3) = t;
        
        m_state.service_received = true;
        m_state.localize_success = false;
        
        RCLCPP_INFO(this->get_logger(), "🎯 Received Initial Pose from RViz! Triggering relocalization...");
    }
    // ===================================================================
    ...
    void timerCB()
    {
        ...
        if (result)
        {
            ...
            if (!m_state.localize_success && m_state.service_received)
            {
                ...
                m_state.service_received = false;
                RCLCPP_INFO(this->get_logger(), "✅ Relocalization MATCH SUCCESS!");
            }
        }
    ...
    void syncCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg, const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg)
    {
        ...
        if (!m_state.message_received)
        {
            ...
            // Keep map->local TF consistent with odometry parent frame.
            // This frame is used in localization math as the local map frame.
