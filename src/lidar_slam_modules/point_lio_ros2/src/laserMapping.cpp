...
void livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::SharedPtr msg) {
    ...
    for(int i = 0; i < plsize; i++) {
        // 0.5米盲区过滤，保护 Point-LIO 不崩溃
        if((msg->points[i].x * msg->points[i].x + msg->points[i].y * msg->points[i].y + msg->points[i].z * msg->points[i].z) < 0.25) continue;
        ...
        pt.curvature = msg->points[i].offset_time / 1e6; // 将纳秒转为毫秒
        ptr->push_back(pt);
    }
    // ----------------------------------------------------
